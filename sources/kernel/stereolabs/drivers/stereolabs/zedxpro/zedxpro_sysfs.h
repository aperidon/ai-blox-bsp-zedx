...
#ifndef __ZEDXPRO_SYSFS_H__
#define __ZEDXPRO_SYSFS_H__
#if 0
/**
...
{
	struct zedxpro *priv = container_of(kobj, struct zedxpro, debug_sysfs);
	struct tegracam_device *tc_dev = priv->tc_dev;
	struct device *dev = tc_dev->dev;
	

	dev_info(dev, "%s: User read requested \n", __func__);
	zedxpro_read_reg(priv->s_data,(u16)priv->register_,(u8*)&priv->value_);
		...
		struct attribute *attr,const char *buf, size_t count)
{
	...
	if(i==2)
		zedxpro_write_reg(priv->s_data,(u16)priv->register_,(u8)priv->value_);
 ...
 */
static struct sysfs_ops zedxpro_sensor_sysfs_ops = {
    .show = sysfs_regs_show,
    .store = sysfs_regs_store,
};

static struct attribute zedxpro_sensor_flash_attribute = {
    .name = "sensor_regs",
    .mode = 0666, /* all users can set the test mode, without issues */
};

static struct attribute *zedxpro_sensor_attrs[] = {
    &zedxpro_sensor_flash_attribute,
    NULL,
};
 ...
 */
static struct kobj_type zedxpro_sensor_kobj_type = {
	.sysfs_ops = &zedxpro_sensor_sysfs_ops,
	.default_attrs = zedxpro_sensor_attrs,
};
