...
static void print_sensor_info(struct max96712 *priv, struct sensor *sp)
{
...
static inline void model_reset(struct max96712 *priv, u8 model)
{
...
static int setup_sensor_pipe(struct max96712 *priv, struct sensor *sp, u8 ser_pipe)
{
	...
	/* for zedlink duo, we are only outputing on MIPI B when using the
	 * 2x15 pins output for NX/NANO devkits */
//	if (priv->csi_port == 'b')
//		offset = 0x02;

	/* patch for the dual deserializer because we have only 2 VIs for
	 * serial_[e-h]. problematic in 4x2 */
	// if ((sp->dser_port==1) && priv->channel == 1)
	// 	offset = 0x01;
...
//		offset = 0x03*2;
	...
	/* patch for the dual deserializer because we have only 2 VIs for
	 * serial_[e-h], serial_e gets PHY0 and here, serial_g (deser mipi port B)
	 * gets PHY2 */
//	if ((sp->dser_port == 1) && priv->channel == 1)
//		offset = 0x03;
                         ...
                         int model, u8 link)
{
    ...
    dev_dbg(&client->dev, "%s: n_cam = %d ->  cam_pipping = 0x%x  (i2c_bus = %x)\n",
                     __func__, priv->n_cam, cam_pipping, sp->i2c_bus);
...
static int sl_max96712_gmsl_pipeline_setup(struct max96712 *priv)
{
    ...
    for (i = 0; i < N_GMSL_PORTS; i++)
    {
        ...
        list_for_each(pos, &priv->sensor_list)
        {
            ...
            if (!(sp->model == model) || sp->detection_id >= 0)
                continue;

            /* now this camera will get the right i2c */
			// if (priv->avail_i2c_bus[sp->i2c_bus] >= 2)
			// {
			// 	dev_warn(&client->dev, "%s: Bus %d not available for camera model %d\n",
			// 			__func__, priv->avail_i2c_bus[sp->i2c_bus], model);
			// 	continue;
			// }

            // if (priv->avail_i2c_bus[sp->i2c_bus] != 0 && priv->avail_i2c_bus[sp->i2c_bus] > priv->avail_i2c_bus[1])
            //     continue;
        ...
        priv->n_cam++;
        dev_dbg(&client->dev, "%s: device channel : %d / id : %d\n", __func__, priv->channel, sp->zedx_id);
        dev_dbg(&client->dev, "%s: avail_i2c_bus = [ %d , %d ] -> isSecondCam : %d\n", __func__, priv->avail_i2c_bus[0], priv->avail_i2c_bus[1], sp->is_second_cam_from_i2c);
        dev_dbg(&client->dev, "%s: csi route : dser port %d -> %d jetson serial",__func__, sp->dser_port, sp->serial);
...
static int slow_reset_Dser(struct max96712 *priv)
{
    int err;
	...
	err = sl_max96712_write_table(global_priv[channel], mode_table[MAX96712_CSI_B]);
...
int getCamPipeIndex(int channel, int zedx_id)
{
    ...
    for( i=0; i < global_priv[channel]->n_cam; i++)
    {
        ...
        if( zedx_id == sp->zedx_id)
        {
            ...
            {
                if(sp->pipes[j] >= 0)
                    return j; //Only works when one pipe used (not for stereo cam)
            }
            return -1;
        }
    }
    ...
    for (i = 0; i<n_sensors ; i++)
    {
        ...
        if (cam_node == NULL)
        {
            dev_info(&i2c_client->dev,
                     "%s: no camera-sensors node, assuming raw10 dtype\n", __func__);
            return 0;
        }

        err = of_property_read_string(cam_node, "data-type", &str);

        if (err)
            dev_info(&i2c_client->dev,
                     "%s: no datatype specified, assuming raw10 dtype\n", __func__);

        dev_info(&i2c_client->dev,
                 "%s: %s datatype selected\n", __func__, str);
        ...
        const struct i2c_device_id *id)
{
	...
	/*
	enable_all_port_Dser(priv->channel);

	links_check_Dser(priv->channel);

	enable_all_port_Dser(priv->channel);
	*/
	sl_max96712_gmsl_pipeline_setup(priv);
