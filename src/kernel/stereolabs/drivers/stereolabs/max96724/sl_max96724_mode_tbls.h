/**
 * max96724_mode_tbls.h - Deserializer mode tables
 *
...
#define SLEEP 0xf1f1
#define MAX96724_TABLE_END 0xff01
...
// MAX96724 i2c address
#define SOURCE_ID 0x01

// MAX96724 number of GMSL ports 
#define N_GMSL_PORTS 4
#define N_GMSL_TRANS 2
#define N_SER_PIPES 4
#define N_DSER_PIPES 4
#define N_DSER_OPT_PORTS 4
...
#define MAX96724_FSYNC_CONF		0x04A0
...
#define N_MAX_TOTAL_SER 8 /* number of ser in DT */
...
void get_zedxnano_alt_fingerprint(struct i2c_fingerprint* table, const size_t size, const int ser_addr, const int left_sensor_addr, const int right_sensor_addr);
...
void get_zedxnano_alt_fingerprint(struct i2c_fingerprint* table, const size_t size,const int ser_addr, const int left_sensor_addr, const int right_sensor_addr){
	struct i2c_fingerprint zedxnano_alt_fingerprint[] = {
		{0x62, 				0x0000, ser_addr<<1}, // Set serializer addr 
		{ser_addr, 			0x000D, 0x95}, // Fix addr
		{0x10, 				0x301B, 0x50}, // Image sensor @ x10 unlock reg 
		{0x10, 				0x31FD, left_sensor_addr<<1}, // Image sensor @ x10 change addr 
	...
	if(size < sizeof(zedxnano_alt_fingerprint)){
		printk(KERN_ERR "zedxnano_alt_fingerprint: table size too small %zu < %zu\n", size, sizeof(zedxnano_alt_fingerprint));
	}

	memcpy(table, zedxnano_alt_fingerprint, sizeof(zedxnano_alt_fingerprint));
}
...
static struct index_reg_8 zedx_mappings[] = {
	...
	{ 0x0415, 0x39}, /* 2500Mbps for zedx (rate exonerated for the concurrency corruption:
			  * identical salad at 1600, root cause was STREAM_SELECT_ALL - see 0x00F4) */
...
static struct index_reg_8 zedxnano_mappings[] = {
	/* First mapping register is the MIPI mapping,
	 * it depends on the buswidth of the deserializer *
	 * The address and the port might change*/
	{ 0x092D, 0x00}, // All mappings to controller 0 (port B/E)
	{ 0x0415, 0x39}, // 2500Mbps for zedxnano
...
static struct index_reg_8 max96724_fast_reset[] = {
...
static struct index_reg_8 max96724_link_regs[] = {
...
static struct index_reg_8 max96724_7_fps[] = {
...
static struct index_reg_8 max96724_15_fps[] = {
...
static struct index_reg_8 max96724_25_fps[] = {
...
static struct index_reg_8 max96724_30_fps[] = {
...
static struct index_reg_8 max96724_60_fps[] = {
...
static struct index_reg_8 max96724_120_fps[] = {
...
static struct index_reg_8 max96724_200_fps[] = {
	{0x04A5, 0x48}, // frame rate
	{0x04A6, 0xE8}, // frame rate
	{0x04A7, 0x01}, // frame rate
	{MAX96724_TABLE_END, 0x00}
};

// TODO: More comments about the registers
// Initialization that is camera independent.
// Rename into max96724_2x4_init_table.
// Create max96724_4x2_init_table.
static struct index_reg_8 max96724_init_table[] = {
	// Full reset, registers and paths
	{0x0013, 0x40},
	// The following lines comes from the datasheet
	// and are setting up the max96724. We don't have
	...
	//{MAX96724_ADDRESS, 0x0018, 0x0F}, // Reset everything except the registers
	...
	/* PAR-16: initial auto-deskew, one DESKEW_INIT per MIPI PHY (UG Table 1 step 30,
	 * Table 13). Mandatory above 1.5 Gbps/lane (zedx runs the CSIPLL at 2500).
	 * Must be set before video is received; the pattern then emits automatically
	 * after video lock, before the first HS data. Verified harmless solo+concurrent. */
	{ 0x0903, 0x80},
	{ 0x0943, 0x80},
	{ 0x0983, 0x80},
	{ 0x09C3, 0x80},

	{ 0x04AF, 0xCf}, // AUTO_FS_LINKS = 0, FS_USE_XTAL = 1, FS_LINK_[3:0] = 0
...
static struct index_reg_8 max96724_2x4_init_table[] = {
	// Full reset, registers and paths
	{0x0013, 0x40},
	// The following lines comes from the datasheet
	// and are setting up the max96724. We don't have
	...
	// MIPI PHY configuration
	// {0x08A0, 0x04}, // CSI output is 4x2
	// {0x08A3, 0xE4}, // 4x2 lane mapping
	// {0x08A4, 0xE4}, // 4x2 lane mapping

	// { 0x0415, 0x28},
	// { 0x0418, 0x28}, // 1200Mbps/lane on port D
	// { 0x041B, 0x28}, // 1200Mbps/lane on port E
	// { 0x041E, 0x28},
...
static u8 cam_pipes[] = {
	[ZEDX] = 0x3,		/* ar0234_9295D_Ser_A: 0x0311=0x21 -> X,Y */
	[ZEDONEGS] = 0x1,	/* ar0234_9295A_Ser_A: 0x0311=0x10 -> X   */
	[ZEDONE4K] = 0x2,	/* imx678_9295A_Ser_A starts X and Y; only Y carries data */
	[ZEDONEHDR] = 0x1,
	[ZEDXHDR] = 0x3,	/* stereo on the 9295D tables, same X,Y as ZEDX */
	[ZEDXNANO] = 0x3,
};
...
static struct index_reg_8 *mode_table[] = {
	[MAX96724_INIT] = max96724_init_table,
	[MAX96724_INIT_2x4] = max96724_2x4_init_table,
	[MAX96724_FST_RST] = max96724_fast_reset,
	[MAX96724_LINK_REGS] = max96724_link_regs,
	[MAX96724_7_FPS] = max96724_7_fps,
	[MAX96724_15_FPS] = max96724_15_fps,
	[MAX96724_25_FPS] = max96724_25_fps,
	[MAX96724_30_FPS] = max96724_30_fps,
	[MAX96724_60_FPS] = max96724_60_fps,
	[MAX96724_120_FPS] = max96724_120_fps,
	[MAX96724_200_FPS] = max96724_200_fps,
	[MAX96724_CSI_B] = max96724_csi_b,
};
