                                      ...
                                      struct fifo_burst_fifo_data *fifo_data)
{
    ...
    if (spsc_dev->gyro_sample_interval_ns == 0) {
        if (gyro_count > 5) {
            dev_dbg((struct device*)spsc_dev->device_ptr,
...
struct bmi_spsc_device *bmi_spsc_device_init(int bmi_id, struct bmi_state *st, struct bmi_spsc_fn_dev *fn_dev)
{
     ...
     * Use CLOCK_MONOTONIC so backward clock_settime() on CLOCK_REALTIME (NTP/PTP
     * step, manual date change) does not push the absolute deadline into the
     * future and freeze IMU production for |delta| seconds.  The deadline is
     * computed from ktime_get_ns() in bmi_spsc_start(), which is monotonic, so
     * the timer's clock base must match. */
    hrtimer_init(&spsc_dev->timer, CLOCK_MONOTONIC, HRTIMER_MODE_ABS_PINNED);
