 ...
 * @priv: deserializer private data (struct max96712 *)
...
{
    struct max96712 *deser_priv = (struct max96712 *)priv;
    u64 sum_bps = 0ULL;
    int i;
 ...
 */
static int sl_get_phy_index_from_csi_group(struct max96712 *priv, int csi_group)
{
	int phy_idx;
 ...
 */
static u64 sl_get_csi_capacity(struct max96712 *priv, int csi_group)
{
	u64 capacity_bps;
	int phy_idx;
...
int sl_get_bw_status(struct device *dev, void *priv, int csi_group)
{
	...
	deser_priv = (struct max96712 *)priv;
	if (!deser_priv || !dev)
...
static inline void reset_all_serializers(struct max96712 *priv)
{
    int j;
    int err = 0;
	struct i2c_client *client = priv->i2c_client;
...
static inline void reset_all_models(struct max96712 *priv)
{
	u8 model;
	int err;
...
static int identify_via_eeprom(struct max96712 *priv)
{
	struct i2c_client *client = priv->i2c_client;
...
static inline int match_fingerprint(struct max96712 *priv, u8 model)
{
...
static bool gmsl_link_present(struct max96712 *priv, u8 i, int tab_id)
{
	struct i2c_client *client = priv->i2c_client;
	unsigned int link = 0;
 ...
 */
static int sl_max96712_poll_cc_lock(struct max96712 *priv, u8 port, int tab_id)
{
	struct i2c_client *client = priv->i2c_client;
...
static int sl_max96712_gmsl_pipeline_setup(struct max96712 *priv)
{
    ...
    for (i = 0; i < N_GMSL_PORTS; i++)
    {
        ...
        if (sl_max96712_poll_cc_lock(priv, i, tab_id)) {
            dev_dbg(&client->dev,
    ...
    err = sl_max96712_i2c_setup(priv);
    if(err)
    {
...
static int sl_max96712_configure_sync_mode(struct max96712 *priv){
    ...
    switch (priv->mfp_trig_out) {
        case 10:
            err = regmap_write(priv->regmap, MAX96712_FSYNC_CONF, 0x24); // MFP10 as output
            break;
        case 2:
            err = regmap_write(priv->regmap, MAX96712_FSYNC_CONF, 0x04); // MFP2 as output
            break;
        default:
            dev_err(&priv->i2c_client->dev, "%s: Invalid mfp_trig_out value (%d) - Defaulting to MFP10 \n",
                __func__, priv->mfp_trig_out);
            err = regmap_write(priv->regmap, MAX96712_FSYNC_CONF, 0x24); // Default to MFP10 as output
    }
    if(err < 0){
	...
	if(hw_rqst_slave_mode || sync_mode > 0) {
		...
		dev_info(&priv->i2c_client->dev, "Sync mode configured - Deser set as Slave\n");
	}
...
static int slow_reset_Dser(int channel)
{
    ...
    {
        unsigned int v;
        if (!regmap_read(global_priv[channel]->regmap, MAX96712_FSYNC_CONF, &v))
...
int dser_enable_gmsl_link(int channel, int zedx_id){
    ...
    list_for_each(pos, &global_priv[channel]->sensor_list){
        ...
        /* This sleep is recommended by the datasheet but does not seem mandatory */
        // msleep(SLEEP_TIME); 
...
u32 fps_set_Dser(int channel, s64 val)
{
    ...
    err = sl_max96712_write_table(global_priv[channel], fps_arr);
...
int fsync_set_Dser(int channel, bool on)
{
    ...
    err = write_reg_Dser(channel, MAX96712_FSYNC_CONF, val);
    if (!err)
...
#endif
{
    struct max96712 *priv = i2c_get_clientdata(client);
    int i;
