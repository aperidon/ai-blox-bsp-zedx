...
static struct v4l2_ctrl_config cfg_list[] = {
	{
		...
		.min = IMX678_EEPROM_STR_SIZE_PRIMARY,
		.max = IMX678_EEPROM_STR_SIZE_PRIMARY,
		.def = 0,
		.elem_size = 1,
		.step = 1,
	},
...
static int imx678_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
	...
	/* Bandwidth management for dynamic FPS */
	priv->fps = val;
	if (priv->is_streaming && sl_get_deser_priv(priv->channel)
		&& priv->csi_group >= 0) {
		new_bps = (u64)mode->image_properties.width *
			mode->image_properties.height *
			(int)(val / 1000000) * priv->bpp;

		if (new_bps != priv->last_bps) {
			ret = sl_update_camera_bw(priv->channel, priv->zedx_id,
				(u32)priv->i2c_client->addr, new_bps, true);
			if (ret < 0) {
				dev_warn(dev, "%s: bandwidth update failed (%d) on channel %d\n",
					__func__, ret, priv->channel);
			}
		...
		s64 val)
{
	...
	int i = 0;
	int HDR_GAIN_FACTOR = 64;

	if(val <= 0){
		return 0;
	}

	// Dirty workaround to noisy images, to be removed when the integration time is fixed
	if(val > 390 && s_data->mode == 1){
		return 0;
	}

	if(val > 530 && s_data->mode == 5){
		return 0;
	}
	...
	integration_time_line_long = integration_time_line *HDR_GAIN_FACTOR;
    ...
    if (reg_shr1 < IMX678_MIN_SHR0_LENGTH)
		...
		integration_time_line_long = ((rsh1/2) - reg_shr1)*HDR_GAIN_FACTOR;
		}
		...
		case MEDIA_BUS_FMT_SRGGB10_1X10:
			{
				...
				err = set_bitrate_Dser(priv->channel, priv->zedx_id, 10);
		...
		case MEDIA_BUS_FMT_SRGGB12_1X12:
			{
				...
				err = set_bitrate_Dser(priv->channel, priv->zedx_id, 12);
...
static int imx678_start_streaming(struct tegracam_device *tc_dev)
{
	...
	if (sl_get_deser_priv(priv->channel) && priv->csi_group >= 0) {
		u32 fps_val;
		...
		bps_needed = (u64)mode->image_properties.width *
			mode->image_properties.height * fps_val * priv->bpp;
	...
	if (err) {
		...
		if (priv->last_bps > 0 && priv->csi_group >= 0) {
...
static int imx678_stop_streaming(struct tegracam_device *tc_dev)
{
    ...
    deser_priv = sl_get_deser_priv(priv->channel);
    if (deser_priv && priv->csi_group >= 0 && priv->last_bps > 0) {
...
static int imx678_board_setup(struct imx678 *priv)
{
    ...
    if (sl_get_deser_priv(priv->channel)) {
        priv->csi_group = dser_get_csi_group(priv->channel, priv->zedx_id, priv->zed_sysfs.gmsl_port);
    } else {
        priv->csi_group = -1;
    }
	if (priv->csi_group < 0) {
		dev_warn(dev, "Could not determine CSI group for channel %d, zedx_id %d, port %d\n",
		priv->channel, priv->zedx_id, priv->zed_sysfs.gmsl_port);
    } 
	atomic_set(&priv->bw_status, BW_OK);
	priv->last_bps = 0;
	priv->is_streaming = false;
...
int custom_s_ctrl(struct v4l2_ctrl *ctrl){
	...
	uint8_t tmp_eeprom_buff[IMX678_EEPROM_SIZE] = {};

	switch (ctrl->id) {
	case ZED_CAMERA_CID_EEPROM_DATA:
		buf_offset = IMX678_EEPROM_SIZE_PRIMARY;
		max_size = IMX678_EEPROM_SIZE_PRIMARY;
		break;
	case ZED_CAMERA_CID_EEPROM_DATA_FULL:
		buf_offset = 0;
		max_size = IMX678_EEPROM_SIZE;
		break;
		default:
	...
	actual_size = strlen(ctrl->p_new.p_char) / 2;
...
static int subdev_register(struct v4l2_subdev *sd){
	...
	for (i = 0; i < num_ctrls; i++) {
		...
		if(ctrl->id == ZED_CAMERA_CID_EEPROM_DATA || ctrl->id == ZED_CAMERA_CID_EEPROM_DATA_FULL){
			int eeprom_sz = (ctrl->id == ZED_CAMERA_CID_EEPROM_DATA) ? IMX678_EEPROM_SIZE_PRIMARY : IMX678_EEPROM_SIZE;
			int buf_off = (ctrl->id == ZED_CAMERA_CID_EEPROM_DATA) ? IMX678_EEPROM_SIZE_PRIMARY : 0;
...
static void imx678_shutdown(struct i2c_client *client){
	...
	for(i = 0;i<IMX678_EEPROM_NUM_BLOCKS;++i){ // write all EEPROM blocks
		int j;
		for(j = 0;j<(IMX678_EEPROM_BLOCK_SIZE/16);++j){ // eeprom can be written only 16 bytes at a time
			err = regmap_bulk_write(priv->eeprom[i].regmap, 16 * j, &priv->eeprom_buf[i * IMX678_EEPROM_BLOCK_SIZE + 16*j], 16);
			msleep(20);
		}
	}
...
#endif
{
    ...
    if (sl_get_deser_priv(priv->channel) && priv->last_bps > 0 && priv->csi_group >= 0) {
        sl_update_camera_bw(priv->channel, priv->zedx_id, (u32)priv->i2c_client->addr, 0, false);
    }
