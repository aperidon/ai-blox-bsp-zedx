		...
		struct attribute *attr, char *buf)
{
	...
	/* GMSL-only attributes: not applicable on MIPI cameras, report -1
	 * (also keeps the weak serdes imports below unreachable there) */
	if (priv->is_mipi &&
	    (!strcmp(attr->name, "bw_status") || !strcmp(attr->name, "link_lock") || 
		 !strcmp(attr->name, "video_lock") || !strcmp(attr->name, "status")))
		return sprintf(buf, "%d\n", -1);

	if (!strcmp(attr->name, "name"))
		return sprintf(buf, "%s\n", (priv->name));

	else if (!strcmp(attr->name, "bw_status")) {
		void *deser_priv = sl_get_deser_priv(priv->channel);
		if (deser_priv && priv->csi_group >= 0) {
			int status = sl_get_bw_status(priv->tc_dev->dev, deser_priv, priv->csi_group);
			switch (status) {
				case BW_OK: return sprintf(buf, "0\n");
				case BW_OVERFLOW: return sprintf(buf, "1\n");
...
static struct kobj_attribute bw_status_attribute =
	__ATTR(bw_status, 0444, NULL, NULL);
 ...
 */
static void zed_info_kobj_release(struct kobject *kobj)
{
	/* info_kobj is embedded in struct info_sysfs which is itself
	 * embedded in the driver's devres-managed priv, so nothing to free. */
}

#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)

static struct kobj_type zed_info_kobj_type = {
	.release = zed_info_kobj_release,
