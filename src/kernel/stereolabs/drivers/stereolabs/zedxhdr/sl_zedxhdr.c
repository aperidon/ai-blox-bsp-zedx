...
#define ZEDXHDR_DRIVER_VERSION_PATCH 3
...
static struct v4l2_ctrl_config cfg_list[] = {
	{
		...
		.min = ISX031_EEPROM_STR_SIZE_PRIMARY,
		.max = ISX031_EEPROM_STR_SIZE_PRIMARY,
		.def = 0,
		.elem_size = 1,
		.step = 1,
	},
...
static int zedxhdr_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
	...
	if(err == 0xEEEE) {
		...
		dev_err(dev, "%s: failed to set fps %d to deserializer, error %d\n", __func__, ISX031_DEFAULT_FPS, err);
		return err;
	}

	/* fixed 30fps */
	priv->frame_length = ISX031_DEFAULT_FRAME_LENGTH;
    priv->fps = ISX031_DEFAULT_FPS;

    /* Handle dynamic FPS changes during streaming */
	if (priv->is_streaming && sl_get_deser_priv(priv->channel) && tc_dev->s_data
	    && priv->csi_group >= 0) {
...
static int zedxhdr_start_streaming(struct tegracam_device *tc_dev)
{
	...
	if (sl_get_deser_priv(priv->channel) && tc_dev->s_data
		&& priv->csi_group >= 0) {
		u32 fps_val;
		...
		bps_needed = (u64)mode->image_properties.width *
			mode->image_properties.height * fps_val * BPP_HDR;
...
static int zedxhdr_stop_streaming(struct tegracam_device *tc_dev)
{
    ...
    priv->is_streaming = false;
    if (priv->last_bps > 0 && sl_get_deser_priv(priv->channel) && priv->csi_group >= 0) {
...
int custom_s_ctrl(struct v4l2_ctrl *ctrl){
	...
	uint8_t tmp_eeprom_buff[ISX031_EEPROM_SIZE] = {};

	switch (ctrl->id) {
	case ZED_CAMERA_CID_EEPROM_DATA:
		// For the HDR the primary eeprom is the first 256 bytes (first i2c address)
		buf_offset = 0;
		max_size = ISX031_EEPROM_SIZE_PRIMARY;
		break;
	case ZED_CAMERA_CID_EEPROM_DATA_FULL:
		buf_offset = 0;
		max_size = ISX031_EEPROM_SIZE;
		break;
		default:
	...
	for (i = 0; i < num_ctrls; i++) {
		...
		if (ctrl == NULL) {
			dev_err(&client->dev, "Failed to init %s ctrl, handler.error=%d\n",
					cfg_list[i].name, priv->ctrl_handler.error);
			continue;
		}
		...
		if(ctrl->id == ZED_CAMERA_CID_EEPROM_DATA || ctrl->id == ZED_CAMERA_CID_EEPROM_DATA_FULL){
			int eeprom_sz = (ctrl->id == ZED_CAMERA_CID_EEPROM_DATA) ? ISX031_EEPROM_SIZE_PRIMARY : ISX031_EEPROM_SIZE;
			// For the HDR the primary eeprom is the first 256 bytes (first i2c address)
			int buf_off = 0;
...
static void zedxhdr_shutdown(struct i2c_client *client){
	...
	for(i = 0;i<ISX031_EEPROM_NUM_BLOCKS;++i){ // write all EEPROM blocks
		int j;
		for(j = 0;j<(ISX031_EEPROM_BLOCK_SIZE/16);++j){ // eeprom can be written only 16 bytes at a time
			err = regmap_bulk_write(priv->eeprom[i].regmap, 16 * j, &priv->eeprom_buf[i * ISX031_EEPROM_BLOCK_SIZE + 16*j], 16);
			if(err)
