...
#define ISX031_IOCTL_SET_MODE			_IOW('o', 1, struct zedxhdr_mode)
...
#define ISX031_IOCTL_GET_SENSORDATA		_IOR('o', 6, \
	 struct zedxhdr_sensordata)
#define ISX031_IOCTL_SET_GROUP_HOLD		_IOW('o', 7, struct zedxhdr_ae)
#define ISX031_IOCTL_SET_HDR_COARSE_TIME	_IOW('o', 8, struct zedxhdr_hdr)
...
#define ZEDXPRO_DRIVER_VERSION_MINOR 4
#define ZEDXPRO_DRIVER_VERSION_PATCH 3

#define ISX031_EEPROM_ADDRESS			0x57
#define ISX031_EEPROM_SIZE				512
#define ISX031_EEPROM_STR_SIZE			(ISX031_EEPROM_SIZE * 2)
#define ISX031_EEPROM_SIZE_PRIMARY			256
#define ISX031_EEPROM_STR_SIZE_PRIMARY		(ISX031_EEPROM_SIZE_PRIMARY * 2)
 ...
 */
static s8 zedxhdr_probe_count = 0;
static struct zedxhdr *zedxhdr_array[MAX_CAM_NUMBER];
...
struct zedxhdr_platform_data {
	const char *mclk_name; /* NULL for default default_mclk */
	unsigned int cam1_gpio;
	unsigned int reset_gpio;
	unsigned int af_gpio;
	bool ext_reg;
	int (*power_on)(struct zedxhdr_power_rail *pw);
	int (*power_off)(struct zedxhdr_power_rail *pw);
};
#endif /* __KERNEL__ */
